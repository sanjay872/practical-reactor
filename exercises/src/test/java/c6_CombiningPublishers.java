...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().flatMap((task)->{
           tasksToExecute();
           return task;
        });
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany((data)->data)
                .flatMap(Mono::just);
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().map((task)->{
          String id=task.block();
            commitTask(id);
            return id;
        });
    ...
    public void car_factory() {
        //todo: feel free to change code as you need
        Flux<Car> producedCars = Flux.zip(carChassisProducer(),carEngineProducer()).map((data)->new Car(data.getT1(), data.getT2()));
